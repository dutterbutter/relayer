 ...
 *
 */
// This is Relayer configuration tests.
// Here we will configure relayer only for private transaction relaying
...
describe('Private Transaction Relaying Service <<>> Withdrawal Config set', function () {
  ...
  it('should relay transaction privately', async () => {
...
describe('Private Transaction Relaying Service <<>> Withdrawal Config not set', function () {
  ...
  // should fail to relay transaction privately as private_tx_relay flag is enabled but withdrawl_config is not set
  it('should fail to relay transaction privately', async () => {
    // we will use chain1 as an example here.
    let anchor1 = await setUpAnchor(signatureBridge, localChain1.chainId);
